...
async function getRunner(
	...
	if (hasDependency("@tauri-apps/cli", root)) {
		if (usesYarn(root)) return new Runner("yarn", ["tauri", "build"]);
		if (usesPnpm(root)) return new Runner("pnpm", ["tauri", "build"]);
		if (usesBun(root)) return new Runner("bun", ["tauri", "build"]);
		return new Runner("npm", ["run", "tauri", "build"]);
	}
